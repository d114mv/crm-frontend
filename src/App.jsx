...
function App() {
  ...
  const [carreras, setCarreras] = useState([]) // 1. Estado para guardar las carreras
  ...
  const [formData, setFormData] = useState({
    ...
    telefono: '',
    estado: 'NUEVO',
    carrera_interes: '' // 2. Ahora enviaremos el ID de la carrera
  })

  // Cargar Leads y Carreras al inicio
  useEffect(() => {
    fetchLeads();
    fetchCarreras();
  }, [])
  ...
  // 3. Función para pedir las carreras a Django
  ...
  return (
    // Usamos min-h-screen para cubrir toda la pantalla con un fondo profesional
                    ...
                    {leads.map((lead) => (
                      <tr key={lead.id} className="hover:bg-slate-700/50 transition duration-150 group">
                        ...
                        <td className="p-4 text-sm text-slate-300">
                          {/* Muestra la carrera o un texto gris si no tiene */}
